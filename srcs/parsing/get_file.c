...
/*   Updated: 2020/02/08 15:15:02 by jecaudal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	is_cub(char *path)
{
	...
	ret = FALSE;
	temp = l_strcut(l_strdup(path), l_strlen(path) - 4);
...
{
	if (sig == LSIG_RESOLUTION && !(file->raw_res = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	else if (sig == LSIG_NPATH && !(file->raw_n_path = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	else if (sig == LSIG_SPATH && !(file->raw_s_path = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	else if (sig == LSIG_EPATH && !(file->raw_e_path = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	else if (sig == LSIG_WPATH && !(file->raw_w_path = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	else if (sig == 64 && !(file->raw_sprite_path = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	else if (sig == 128 && !(file->raw_floor_color = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	else if (sig == 256 && !(file->raw_ceiling_color = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	else if (sig == LSIG_MAP && !(file->raw_map = l_strdupdel(line)))
		return (ERR_MEMALLOC);
	return (SUCCESS);
}
...
t_bool			get_file(t_file *file, char *path)
{
	...
	while (get_next_line(file->fd, &readed) > 0 && (sig = detect_line(readed)))
	{
		if ((file->curr_line++ > 0) && sig == LSIG_UNKNOW)
